...
export const runnerGroupId = 17;
export const createdScaleSetId = 4321;
...
export const staticActionsServiceUrl =
  "https://static-actions.example/tenant";
export const liveActionsServiceUrl = "https://actions.example/tenant";

const createScaleSetScript = join(
  repositoryRoot,
  "scripts/create-scale-set.sh",
);
...
export function validEntry(overrides = {}) {
  return {
    ...
    actionsServiceUrl: staticActionsServiceUrl,
...
export function expectedRequestBody({
  labels = [scaleSetName],
  usedRunnerGroupId = runnerGroupId,
} = {}) {
  return JSON.stringify({
    name: scaleSetName,
    runnerGroupId: usedRunnerGroupId,
    labels: labels.map((name) => ({ type: "System", name })),
...
export function applyResponses({
  scaleSets = [],
  createdId = createdScaleSetId,
} = {}) {
  return [
    ...liveResponses(scaleSets),
    { status: 200, body: JSON.stringify({ id: createdId }) },
  ];
}

export function runCreateScaleSet({
  config = keyedConfig(),
  ...
  const temporaryDirectory = mkdtempSync(
    join(tmpdir(), "create-scale-set-test-"),
  );
  ...
  const scriptArgs = [createScaleSetScript, "--scale-set", scaleSet];
